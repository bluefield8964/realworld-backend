...
public class InvoiceFinalizedHandler implements WebhookHandler {
    private final WebhookAckService webhookAckService;

    @Override
    public BusinessEventType supports() {
        return BusinessEventType.INVOICE_FINALIZED;
    }

    @Override
    public void handle(WebhookContext ctx) {
        webhookAckService.accept(ctx, "invoice.finalized");
    }
}
