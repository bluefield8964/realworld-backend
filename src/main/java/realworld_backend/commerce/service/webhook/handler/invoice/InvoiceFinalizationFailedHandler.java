...
public class InvoiceFinalizationFailedHandler implements WebhookHandler {
    private final WebhookAckService webhookAckService;

    @Override
    public BusinessEventType supports() {
        return BusinessEventType.INVOICE_FINALIZATION_FAILED;
    }

    @Override
    public void handle(WebhookContext ctx) {
        webhookAckService.accept(ctx, "invoice.finalization_failed");
    }
}
